...
test('validateTransaction - valid transaction', () => {
  const result = validateTransaction('2025-12-12', 'Description', 'Category', '-5000,00 PLN', 'valid line')
...
test('validateTransaction - missing date', () => {
  const result = validateTransaction('', 'Description', 'Category', '-5000,00 PLN', 'line')
  assert.strictEqual(result.isValid, false)
  assert.strictEqual(result.error, 'Date is required')
})

test('validateTransaction - invalid date', () => {
  const result = validateTransaction('invalid-date', 'Description', 'Category', '-5000,00 PLN', 'line')
  assert.strictEqual(result.isValid, false)
  assert.strictEqual(result.error, 'Invalid date format: invalid-date')
})

test('validateTransaction - missing description', () => {
  const result = validateTransaction('2025-12-12', '', 'Category', '-5000,00 PLN', 'line')
  assert.strictEqual(result.isValid, false)
...
test('validateTransaction - missing amount', () => {
  const result = validateTransaction('2025-12-12', 'Description', 'Category', '', 'line')
  assert.strictEqual(result.isValid, false)
  assert.strictEqual(result.error, 'Amount is required')
})

test('validateTransaction - invalid amount', () => {
  const result = validateTransaction('2025-12-12', 'Description', 'Category', 'invalid-amount', 'line')
  assert.strictEqual(result.isValid, false)
